package pl.wolny.kwadratowamasakratablist.model.player

import com.comphenix.protocol.wrappers.WrappedChatComponent
import com.comphenix.protocol.wrappers.WrappedSignedProperty

class EmptyTabListPlayer: TabListPlayer {

    companion object{
        private const val skinValue =
            ""
        private const val skinSignature =
            ""
    }

    override fun skin(): WrappedSignedProperty {
        return WrappedSignedProperty("textures", skinValue, skinSignature)
    }

    override fun name(): WrappedChatComponent {
        return WrappedChatComponent.fromText(" ")
    }
}